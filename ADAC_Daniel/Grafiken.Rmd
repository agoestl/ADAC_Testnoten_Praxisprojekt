...
data %>% 
  ...
  ggplot(aes(x = deutscher.Hersteller, y = Gesamtpunktzahl)) +
  geom_col() +
  labs(
    title = "Vergleich zwischen deutschen und ausländischen Herstellern",
    x = "Hersteller",
...
# Gesamtpunktzahl nach deutscher.Hersteller (geom_boxplot)
data %>% 
  ggplot(aes(x = deutscher.Hersteller, y = Gesamtpunktzahl)) +
  geom_boxplot() +
  labs(
...
data %>% 
  ...
  geom_col() +
  labs(
    title = "Durschnittliche Gesamtpunktzahl nach CO2-Effizienzklasse",
    x = "CO2-Effizienzklasse",
    y = "durchschnittliche Gesamtpunktzahl"
  )

# Gesamtpunktzahl nach CO2-Effizienzklasse (geom_boxplot)
data %>% 
  drop_na(CO2.Effizienzklasse) %>% 
  ggplot(aes(x = fct_relevel(CO2.Effizienzklasse, "A+"), y = Gesamtpunktzahl)) +
  geom_boxplot() +
...
data %>% 
  ...
  geom_col() +
  labs(
    title = "Durschnittliche Gesamtpunktzahl nach Motorart",
    y = "durchschnittliche Gesamtpunktzahl"
  )
...
# Batteriekapazität nach Gesamtpunktzahl für Elektroautos
data %>% 
  drop_na(Batteriekapazität..Netto..in.kWh) %>% 
  group_by(Gesamtpunktzahl) %>% 
  mutate(Batteriekapazität..Netto..in.kWh = mean(Batteriekapazität..Netto..in.kWh)) %>% 
  ggplot(aes(x = Gesamtpunktzahl, y = Batteriekapazität..Netto..in.kWh)) +
  geom_point() +
  geom_smooth(method = "loess", se = FALSE, color = "red") +
  labs(
    title = "Batteriekapazität nach Gesamtpunktzahl für Elektroautos",
...
Man erkennt, dass es einen Zusammenhang zwischen Reichweite von Elektroautos und der GEsamtpunktzahl gibt.
Tendenziell hat eine höhere Reichweite eine höhere Punktzahl, wobei es bei sehr hohen Punktzahlen nicht mehr unbedingt stimmt.
Denselben Zusammenhang kann man auch zwischen der Batteriekapazität von Elektroautos und der Gesamtpunktzahl erkennen.