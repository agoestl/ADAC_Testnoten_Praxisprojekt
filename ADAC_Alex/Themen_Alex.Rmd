...
data %>% 
  mutate(Laufendekosten = sum(c(Betriebskosten, Werkstattkosten, Fixkosten, KFZ.Steuer.ohne.Befreiung.Jahr..kann.durch.WLTP.abweichen., Haftpflichtbeitrag.100.), na.rm = TRUE)) %>% 
  ggplot(aes(x = Motorart, y = Laufendekosten, color = Laufendekosten)) +
  geom_col() + 
  labs(title = "relativ Laufende Kosten der verschiedenen Motorarten", x = "Motorart", y = "Laufendekosten in € (anteilsmäßig)")
